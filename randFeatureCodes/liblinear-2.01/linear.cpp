...
{
	public:
		Solver_MCSVM_CS(const problem *prob, Int nr_class, double *C, double eps=0.1, Int max_iter=100);
		~Solver_MCSVM_CS();
		void Solve(double *w);
	private:
...
void Solver_MCSVM_CS::solve_sub_problem(double A_i, Int yi, double C_yi, Int active_i, double *alpha_new)
{
	...
	//std::sort( D, D+active_i, compare_double );
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	for(Int i=0;i<prob->l;i++)
		if(prob->y[i] > 0)
			pos++;
	neg = prob->l - pos;
	double primal_solver_tol = eps*max(min(pos,neg), (long)1)/prob->l;
